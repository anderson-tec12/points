...
function pointsRoutes(knex2) {
  return async (app2) => {
    app2.get(
      ...
      async (request, reply) => {
        try {
          ...
          const pointsService = new PointsService(knex2);
    ...
    app2.get("/resume", async (request, reply) => {
      const pointsService = new PointsService(knex2);
      const userService = new UserService(knex2);
      const monthCurrent = dateUtils.getMonthAndYear();
...
function usersRoutes(knex2) {
  return async (app2) => {
    ...
    app2.post("/list", async (request, reply) => {
      try {
        const createUsersBodySchema = import_zod2.z.object({
          usersName: import_zod2.z.string().array()
        });
